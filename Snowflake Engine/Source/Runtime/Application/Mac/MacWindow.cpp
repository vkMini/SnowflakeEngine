...
namespace Snowflake {
	...
	MacWindow::MacWindow(const WindowSpecification& specs)
	...
	std::pair<uint32_t, uint32_t> MacWindow::GetPosition() const
	{
		int xPos, yPos;
	...
	void MacWindow::SetVSyncEnabled(bool isEnabled)
	{
		glfwSwapInterval(isEnabled ? GLFW_TRUE : GLFW_FALSE);
		m_WindowData.VSync = isEnabled;
	}

	void MacWindow::SetResizable(bool isResizable)
	{
